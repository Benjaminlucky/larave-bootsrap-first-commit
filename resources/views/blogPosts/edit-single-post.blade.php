                    ...
                    You're logged in!
                        ...
                        <div class="row postFormWrp">
                            <div class="col-md-6 mx-auto postFormCon">
                                ...
                                @endif
                                <form action="{{route('blog.update', $post)}}" method="post" class="postCreate" enctype="multipart/form-data">
                                    @method('put')
                                    @csrf
                                    <div class="postFormInput">
                                        <input type="text" class="postInput" name="postTitle" placeholder="Write post title" value="{{$post->title}}">
                                        @error('postTitle')
                                    ...
                                    <div class="postFormBody" id="body">
                                        <textarea name="body" id="lucky_summernote" placeholder="Write Post">{{$post->body}}</textarea>
                                        @error('body')
                                            <p class="validationError">{{$message}}</p>
                                        @enderror
                                    </div>
